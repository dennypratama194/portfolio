...
**The `.btn` system is the only button pattern for public pages.** One primary per view, secondary beside it, `.btn-on-dark` when the section background is `--ink`. Both variants share padding, radius, border width and type size, so they always line up; hover is one language everywhere — the button steps forward: the primary lifts on a red glow, the secondary's outline and label come up to full contrast. **No fill swaps on hover.** A filled hover on a red button leaves the white label nothing to sit on, and the custom cursor's filled ring is already 64px against a ~53px button — `body.cursor-on-btn` shrinks it back to an outline over any `.btn` so it never covers the text. Never restyle a button inline or re-declare its padding per section — that is how the old four-class set drifted into two different components.
